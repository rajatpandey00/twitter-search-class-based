...
export const reducer = (state, action) => {
    ...
    else if (action.type === 'FETCH_TWEETS_FAILED')
        return state
...
// @flow
export default ({ profiles, viewProfile, showProfile, userDetails, showSpinner }) => {
    const [tweetsFetched, setTweetsFetched] = useState(false);
    const [data, dispatch] = useReducer(reducer, initialData);
    const [tweets, setTweets] = useState([]);

    const fetchTweets = (screeName) => {
        axios.get(`http://localhost:3000/getUserTweets/${screeName}`)
            .then(({ data }) => {
                dispatch({ type: 'FETCH_TWEETS_SUCCESS', tweets: setTweets(data), tweetsFetched: setTweetsFetched(true) })
            }).catch(() => {
            ...
            <Container fluid>
                <Row>
                    <Col xs={4} className="list-profile">
                        ...
                        <br />
                        {!showSpinner && <Spinner style={{ alignSelf: "center" }} animation="border" variant="primary" />}
                        {profiles.length > 0 &&
                    ...
                    <Col className="profile">
                        {showProfile &&
                            <Profile userDetails={userDetails} tweets={tweets} tweetsFetched={tweetsFetched} />
                        }
                    </Col>
                </Row>
            </Container>
